...
function getModelLeafNodes(rootId, leafNodes, callback) {
    ...
    instanceTree.enumNodeChildren(rootId, function(childId){
        ...
        var childId_arbol = String(childId);
        if (FORGE_dbIds.length > 0)
        {
            //var idx = instanceTree.nodeAccess.dbIdToIndex[childId_arbol];
            for(i= 0; i< FORGE_dbIds.length;i++)
            {
                //if (FORGE_dbIds[i] == idx)
                if (FORGE_dbIds[i] == childId_arbol)
                {
                    leafNodes.push(childId)
                }
            }
