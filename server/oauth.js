...
router.get('/api/forge/callback/oauth', function (req, res) {
  ...
  //// 0.7.2
  //var req = new forgeSDK.AuthClientThreeLegged(config.credentials.client_id, config.credentials.client_secret, config.callbackURL, ['data:read','data:write'], autoRefresh);
  var req = new forgeSDK.AuthClientThreeLegged(config.credentials.client_id, config.credentials.client_secret, config.callbackURL, config.scopeInternal, autoRefresh);
  // req.generateAuthUrl();

  console.log(code);
  req.getToken(code).then(function (internalCredentials) {
      ...
      var req2 = new forgeSDK.AuthClientThreeLegged(config.credentials.client_id, config.credentials.client_secret, config.callbackURL, config.scopePublic, autoRefresh);
